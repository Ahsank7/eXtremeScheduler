...
const Employees = () => {
  const tableColumns = ["Name", "Email", "Company"];
  const [opened, { open, close }] = useDisclosure(false);

  const handleConfirmationModal = (isConform) => {
  ...
  return (
    <>
      <AppContainer
        title="Employees"
        button={<Button onClick={open}>Create Employees</Button>}
        showDivider="true"
      >
        <AppTable thead={tableColumns} totalPages={10} currentPage={1}>
          {data.map((row) => (
            <tr key={row.name}>
              <td>{row.name}</td>
              <td>{row.email}</td>
      ...
      {/* <AppConfirmationModal opened={opened} onClose={handleConfirmationModal} /> */}

      {/* <AppModal opened={opened} size="lg" onClose={close} title="Add Employee">
        <form>
          <TextInput
            withAsterisk
            label="Email address"
